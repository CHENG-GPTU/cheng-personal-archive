...
// React Bits official TS-CSS source; see LICENSE.md and SOURCE.md.
...
const SideRays = ({
  ...
  useEffect(() => {
    ...
    // Integration guard: a stale deferred effect must never create a second canvas.
    let disposed = false;
...
void main() {
  ...
  gl_FragColor = color;
      ...
      updateSize();
      animationIdRef.current = requestAnimationFrame(loop);
      containerRef.current.dataset.webgl = 'ready';
      const contextLost = () => cleanupFunctionRef.current?.();
      ...
      cleanupFunctionRef.current = () => {
        ...
        if (containerRef.current) containerRef.current.dataset.webgl = 'released';
    ...
    const initializeTimer = window.setTimeout(() => {
      try {
        ...
        // No WebGL support: retain the site's original background and content.
        cleanupFunctionRef.current?.();
        ...
        uniformsRef.current = null;
        meshRef.current = null;
        if (containerRef.current) containerRef.current.dataset.webgl = 'unavailable';
      }
    }, 10);
